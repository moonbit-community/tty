...
| COLOR-1 | done | semantic color package and output command helpers | `docs/plans/2026-05-20-color-output-api.md`, `color/`, `vt/`, root package, `cmd/color`, `cmd/agent` | `color/` owns semantic color values, `vt` exposes low-level SGR builders, and `Output` has command-style style/screen helpers | `moon fmt`, `moon test vt`, `moon test color`, `moon test vt/color`, `moon test .`, `moon check cmd/color`, `moon check cmd/agent`, `moon test`, `moon check`, `moon info` |
